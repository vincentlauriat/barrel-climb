...
final class FireballTests: XCTestCase {
    func worldWithFireball(x: Double, girder: Int, direction: Direction, random: RandomSource) -> World {
        ...
        w.fireballs.append(
            Fireball(
                id: 7, position: Vector2(x: x, y: Levels.barrels.girders[girder].surfaceY(at: x)),
    ...
    func testFireballMovesAtItsOwnSpeedAlongTheGirder() {
        ...
        w.run(60)
        XCTAssertEqual(
            w.fireballs[0].position.x, 40 + Tuning.barrelSpeedPointsPerSecond * Tuning.fireballSpeedFactor,
            accuracy: 1e-6)
