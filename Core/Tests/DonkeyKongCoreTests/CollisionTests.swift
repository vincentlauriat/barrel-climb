...
final class CollisionTests: XCTestCase {
    ...
    /// A ladder is no shelter — a barrel that overlaps the climbing player kills, wherever
    /// on the ladder they are.
    func testBarrelHitsAClimbingPlayerMidLadder() {
        var w = World.playing()
        ...
        w.barrels.append(
            Barrel(
                id: 1, kind: .normal, position: w.player.position,
                direction: .right, currentGirder: nil, currentLadder: 0))
        w.run(1)
        XCTAssertEqual(w.player.state, .dying)
    }
    func testBarrelHitsAPlayerSittingAtTheLadderFoot() {
        var w = World.playing()
    ...
    /// A barrel that does not overlap the player leaves them alone, ladder or not.
    func testBarrelClearOfTheClimbingPlayerDoesNotHit() {
        var w = climbingMidLadder(2)
        w.barrels.append(
            Barrel(
                id: 1, kind: .normal, state: .onLadder,
                position: Vector2(x: w.player.position.x + 40, y: w.player.position.y),
                direction: .right, currentGirder: nil, currentLadder: 2))
        w.run(1)
        XCTAssertEqual(w.game.phase, .playing)
    }
