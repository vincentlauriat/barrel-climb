...
final class BarrelTests: XCTestCase {
    ...
    func testBarrelKeepsDirectionAcrossConnectedGirders() {
        ...
        w.barrels.append(
            Barrel(
                id: 99, kind: .normal, position: Vector2(x: 120, y: Levels.barrels.girders[1].surfaceY(at: 120)),
    ...
    func testBarrelReversesAtTheSceneEdge() {
        ...
        w.barrels.append(
            Barrel(
                id: 99, kind: .normal, position: Vector2(x: 220, y: Levels.barrels.girders[1].surfaceY(at: 220)),
    ...
    func testBarrelIsRemovedInTheOilDrum() {
        ...
        w.barrels.append(
            Barrel(id: 99, kind: .normal, position: Vector2(x: 40, y: 8), direction: .left, currentGirder: 0))
        w.run(60)
        XCTAssertTrue(w.barrels.isEmpty)
    }
}
